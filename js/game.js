...
class Game {
    ...
    showWinDialog(winnerName, result) {
        ...
        message.innerHTML = `
            <p>台數: ${result.fans} 台</p>
            <p>分數: ${result.points}</p>
            <p>${result.details.join('<br>')}</p>
        `;
    ...
    updateUI() {
        const remaining = this.tileSet.remaining();
        document.getElementById('tile-count').textContent = `剩餘牌數: ${remaining}`;
    ...
    createPlayerArea(player) {
        ...
        area.className = `player-area ${player.direction}`;
        
        const handDiv = document.createElement('div');
        handDiv.className = 'player-hand';
        
        const discardsDiv = document.createElement('div');
        discardsDiv.className = 'discard-pile';
